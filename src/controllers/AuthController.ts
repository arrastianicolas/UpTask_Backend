...
export class AuthController {
  ...
  static updateProfile = async (req: Request, res: Response) => {
    ...
    const userExists = await User.findOne({ email });
    //Verifica que exista y que el usuario pueda cambiar nomas el nombre
    ...
    if (req.user.name === name && req.user.email === email) {
      const error = new Error(
        "Cambia tu nombre o email para actualizar tu perfil"
      );
      res.status(409).json({ error: error.message });
      return;
    }
  ...
  static updateCurrentUserPassword = async (req: Request, res: Response) => {
    ...
    if (!isPasswordCorrect) {
      const error = new Error("Contraseña actual es incorrecto");
      res.status(401).json({ error: error.message });
      return;
    }
    try {
